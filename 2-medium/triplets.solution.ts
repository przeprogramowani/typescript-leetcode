...
export function increasingTriplet(nums: number[]): boolean {
  ...
  for (const num of nums) {
    ...
    if (num <= first) {
      first = num;
      // If current number is smaller than second but larger than first, update second
    } else if (num <= second) {
      second = num;
      // If we find a number larger than both first and second, we found our triplet
    } else {
      return true;
    }
  }
